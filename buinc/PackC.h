...
// 函数名: CYHStringMatch
// 编程  : 陈永华 2009-7-15 0:05:36
// 描述  : 判断一个字符串pOrigCode是否匹配模式字符串pDCmpCode的规则范围
// 返回  : bool : 返回字符串pOrigCode是否符合pDCmpCode定义，==0: 表示不符；否则表示符合match
// 参数  : const char *pOrigCode  [IN]: 被判断的字段串
// 参数  : const char *pDCmpCode  [IN]: 匹配模版字符串
/* CYHStringMatch: 是一套用于类似于正则表达式的字符串模式匹配算法
基本规则为: 
   在模式匹配中，如下几个字符为特殊含义符号:
      '?'问号 或'_'下杠号，用于表示任何单字符；
      '*'乘号，用于表示任何字符个数的部分；
      '-'减号，用于表示为空串，即 ""；但在左右方括号内有特殊含义，参见左右方括号
      左右方括号"[...]"内含可以枚举一个字符:
         如: [1359] : 表示该位置的字符可以为 '1','3','5','9'这几个字符;
         若内部包括了'-'，且左边字符小于右侧字符，则表示一个区间字符，如: [1-9] 表示从'1'到'9'(包括这两个)的字符;
      例如: 
         * - any
         [D-F]*4 - D*4,E*4,F*4  
         4* - 4*
         4*0 - 4*0
         4?[D-F] - 4?D,4?E,4?F
         4?[DP-T] - 4?D,4?P,4?Q,4?R,4?S,4?T
         *[D-F][1-9]* - ?D1?,?E2?
      ---------------------------------------------------
      ',' 逗号，用于分割字符串判断单元；常规为只要符合各个判断单元中的其中一个判断单元即为匹配；
         但若判断单元是以'!'开始的，则必须用于排除符合该单元的字符串；
      如下为几个例子：
         pDCmpCode                              pOrigCode
         -------------------------------------  --------------------
         03[0-2][0-9][0-9][0-9]                 即[030000,032999] 如深交所的认购权证代码区间
         03[89][0-9][0-9][0-9]                  即[038000,039999] 如深交所的A 股认沽权证代码区间
         037[0-9][0-9][0-9],!037000             即[037001,037999] 如深交所的上市公司股权激励计划涉及的员工认股权的证券代码区间
         03[3-6][0-9][0-9][0-9]                 即[033000,036999] 如深交所的权证业务预留的代码区间；
*/
PACKC_API bool CYHStringMatch(const char *pOrigCode,const char *pDCmpCode);
...
// 函数名: FilterParmBits
// 编程  : 陈永华 2009-5-19 11:28:16
// 描述  : 将设置的ParmBits进行过滤，
// 返回  : int 返回被过滤出的非法位图个数。=0: 没有异常的位图；>0: 即有多少个被过滤出来的异常位图
// 参数  : unsigned char *parmbits [IN/OUT]: 
PACKC_API int FilterParmBits(unsigned char *parmbits);
