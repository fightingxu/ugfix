...
public:
	...
	int CheckEncodeRows(const void *ebuf, int eblen, const ST_PACKHEAD *phead);
	int CheckParmBits(unsigned char *parmbits);
	int CheckEncodeHead(const void *ebuf, int eblen,ST_PACKHEAD *ph);
...
private:
   ...
   int GetFieldValue(const void *prec, const ST_PDATALIST *pfd, char *value);
	...
	int EncodeSingleField(const ST_PDATALIST *pfd, const void *pdata, void *pbuf, int pbsize);
};
