...
public:
	int FilterParmBit(unsigned char *ParmBits);


   // 函数名: CheckEncodePack
   // 编程  : 陈永华 2008-2-21 12:44:29
   // 描述  : 这个函数专门用于不解包，但检查打包数据内容，是否符合XPACK格式，并返回本打包块的数据实际长度
   // 返回  : int : >0 - 返回打包数据块实际长度; <=0 - 错误格式打包块
   //       -1:eblen ERROR,
   //       -2:INVALID recCount,
   //       -3:INVALID firstflag/nextflag,
   //       -10:INVALID PARMBITS(over field),
   //       -11:INVALID PARMBITS(invalid parmbit)
   //       -20: short of eblen
   //       -21: invalid at a vchar field
   //       -30: too long SDPACK Data
   // 参数  : const void *ebuf [IN]: 打包的原始数据块
   // 参数  : int eblen [IN]: 打包数据长度
	int CheckEncodePack(const void *ebuf, int eblen,ST_PACKHEAD *phead);
