...
private:
   XPROCESS_M pProcess;   
   XCALLBDFUNC_M pCallBDFunc;
   XWRITEAPPFUNC pWriteAppInfo;
...
public:
	void Init(XCALLBDFUNC_M pCallFunc,XWRITEAPPFUNC pWriteApp,XPROCESS_M process);
   ...
   // 参数  : char *oQueueID16 [OUT]: 16字节以上的缓存，用于获取推送请求的QueueID，这个用于匹配原始请求和应答的ID
	...
	int PostBookData(unsigned int postdno, const char *sRUserID, void *pdata, int pdlen, char pushmode, int acktime);
	int PostBookDataX(unsigned int postdno, const TRUSERID *hTRUserID, void *pdata, int pdlen, char pushmode, int acktime);
	int GetReqUserID(TRUSERID *handle, char *sRUserID);
	int GetSvrFuncRegister(int sourceno, int funcno);
	void SetAnswerHead(TRUSERID *handle, ST_PACKHEAD *phead);
	int AnswerDataEx(TRUSERID *ruserid,int len,void *msgbuf);
