...
public:
	int CheckSRAvailableUS(int ustimeout);
	int RecvPartUS(char *vptr, int nMaxSize, int ustimeout);
	int CheckSendAvailableUS(int ustimeout);
	int CheckDataArrivedUS(int ustimeout);
   // 函数名: CTcpSocket::RecvPart
   // 编程  : 陈永华 2004-2-10 21:15:05
   // 描述  : 通过本链路接收数据, 可以小于nMaxSize
   // 返回  : int >0:接收到的数据长度，即nMaxSize；-1:被对方正常关闭；-10:输入的nMaxSize<=0；-2:因为链路错误而被关闭；0:超时；-3:在接收数据的时候，出错而被关闭
   // 参数  : char *vptr ：接收数据的缓存
   // 参数  : int nMaxSize ： 指定接收数据的最大长度
   // 参数  : int timeout : 等待接收数据的超时时间，毫秒计
	int RecvPart(char *vptr, int nMaxSize, int timeout);

	void SetBlockMode(bool bBlock);
	int CheckSendAvailable(int timeout);
	bool IsConnected();
   ...
   int Send(const char *ptr, int nDesignedSize); // 通过本链路发送数据 返回  : int -1:链路没有正常建立，被关闭；-2:发送数据时候出错，被关闭；>=0: 发送指定长度数据成功，应该==nDesignedSize
   ...
   // 参数  : int timeout : 等待接收数据的超时时间，毫秒计
	...
	bool ConnectTcp(const char* host,unsigned short port, int waitms=-1);
/* ****** Updated by CHENYH at 2006-3-10 15:50:33 ****** 
	bool Listen(short iPort);
*******************************************************/
	bool Listen(unsigned short iPort,char *ip=NULL);
...
BUPUB_API void REVDATA(void *data,int len);
