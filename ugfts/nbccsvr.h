/********************************************************************
   ...
   2011-8-18 23:22:38 新增BSTransfer函数
*********************************************************************/
#ifndef NBCCSVR_H
#define NBCCSVR_H
...
// 本模块初始化部分：
   ...
   // 函数名: BSTransfer
   // 编程  : 陈永华 2011-8-18 23:09:00
   // 描述  : 将原始请求转发到另外一个应用服务器(destno:funcno)，由它来处理原始请求并直接应答给客户端
   // 返回  : int <0: 失败，返回错误号，参考errmsg中的信息；否则，成功
   // 参数  : BSHANDLE Handle  [IN] : 当前处理的XPACK数据缓存句柄
   // 参数  : unsigned short destno [IN] : 目标服务器注册的通信平台的branchno (YYBDM)
   // 参数  : unsigned short funcno [IN] : 目标服务器注册的通讯功能号FuncNo
   // 参数  : char *errmsg [OUT] : 当返回码为<0的时候的错误信息
   // 参数  : int emlen [IN] : errormessage的缓存长度
   BCCSVR_API int BSTransfer(BSHANDLE Handle,unsigned short destno, unsigned short funcno, char *errmsg, int emlen);
