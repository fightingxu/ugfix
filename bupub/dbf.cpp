...
#define LOCK_POSITION 0x40000000
#define MAX_FILELEN   0x3FFFFFFF

#define CB_LOCKPOS 0X3B9ACA00  /* For CLIPPER & DBASE4 lock */
...
int CDBF::dbf_update_head()
{
   ...
   month = (BYTE)(ltm.tm_mon+1);  // (BYTE)(ltm.tm_mon) 2006-1-4 17:38:35
...
int CDBF::f4b_binary(char *fieldname, void *bin, int blen)
{
   ...
   /* ****** Updated by CHENYH at 2005-10-9 18:02:31 ****** 
   如果要从别的公司定义的文件格式中读写，－ BIX - CXN要求
   if (f_ptr->f_type!='X')
      return(-3);
   *******************************************************/
   if (blen<=0)
...
int CDBF::f4binary(char *fieldname, void *value, int valuesize)
{
   ...
   if (f_ptr==NULL)
      return(-1);
   /* ****** Updated by CHENYH at 2005-10-9 18:01:21 ****** 
   如果要从别的公司定义的文件格式中读写，－ BIX - CXN要求
      if (f_ptr->f_type!='X')
         return(-3);
   *******************************************************/
   pb = (BYTE *)(buffer+f_ptr->address);
   /* ****** Updated by CHENYH at 2006-7-4 12:05:33 ****** 
   // 避免HP总线对齐异常出现：
   binlen = *(WORD *)(pb);
   if (binlen<valuesize)
   {
   ...
   *******************************************************/
   memcpy(&binlen,pb,sizeof(binlen));
   if (binlen<valuesize)
      valuesize = binlen;
   if (valuesize>0 && value!=NULL)
   {
