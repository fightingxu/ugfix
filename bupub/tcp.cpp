...
void REVDATA(void *data,int len)
{
...
int CTcpSocket::GetLastError(char *szMsgBuf, int MsgBLen)
{
   ...
   if (MsgBLen>0 && szMsgBuf!=NULL)
   {
      ...
      case 6  :  sprintf(msg,"%d:No such device or address",errcode); break;
      ...
      case 14 :  sprintf(msg,"%d:Bad address",errcode); break;
      ...
      case 58 :  sprintf(msg,"errno=%d:Destination address required",errcode); break;
      ...
      case 68 :  sprintf(msg,"errno=%d:Can't assign requested address",errcode); break;
         /* operational errors */
...
bool CTcpSocket::Create(int addressfamily,int type)
{
   ...
   /* ****** Updated by CHENYH at 2009-5-13 23:23:27 ****** 
...
bool CTcpSocket::Listen(unsigned short iPort,char *ip)
{
   if (m_socket==INVALID_SOCKET)
      return(false);
   sockaddr_in addr;
...
bool CTcpSocket::ConnectTcp(const char *host,unsigned short port, int waitms)
{
   unsigned int inaddr;  
   struct hostent *ht;
   ...
   if (waitms>0)
   {
      ...
      if (e<0)
      {
         e = GET_LAST_SOCK_ERROR();
         if ((e==EWOULDBLOCK||e==EINPROGRESS) && CheckSendAvailable(waitms)>0)
         {
            e = 0; // 表示连接成功
         }
         else
	...
	while(RecvLen<nDesignedSize)
	{
      ...
      if (result<0 && GetLastError(szmsg,511)==EWOULDBLOCK)
      {
		...
		else if(result<=0) 
		{
         ...
         if (result==0)
            strcpy(szmsg,"0:be gracefully closed by remote!");
#ifdef WIN32
         _snprintf(vptr,nDesignedSize,"%s",szmsg);
#else
         snprintf(vptr,nDesignedSize,"%s",szmsg);
#endif
			Close();
			RecvLen=-3;
			break;
      ...
      {
			RecvLen+=result;
         // 2011-05-27: 为了防止timeout太小，甚至为0（即快速检查接收数据模式）Add:
         if (timeout<1)
            timeout = 1;
      }
	}
...
int CTcpSocket::Send(const char *ptr, int nDesignedSize)
{
	if(m_socket==INVALID_SOCKET) return -1;
...
//   if (timeout>0)
   ...
   return(1);
#else
   return(CheckDataArrivedUS(timeout*1000));
#endif
}
...
// 2009-5-13 22:18:40 Added for 该端口是否被阻塞的检查 (尤其当对方网络物理断开或机器异常的过长等待 近20秒的阻塞等待)
int CTcpSocket::CheckSendAvailable(int timeout)
{
#if 0
   int result=0;
   if (m_socket==INVALID_SOCKET) 
   ...
   if (timeout>=0)
   {
      ...
      tv.tv_sec = timeout/1000;
      tv.tv_usec = (timeout%1000)*1000;
      result=(select(m_socket+1, NULL, &wset, NULL, &tv));
      if (result<0)
      {
         Close();
         return(-2);
   ...
   return(1);
#else
   return(CheckSendAvailableUS(timeout*1000));
#endif
}

void CTcpSocket::SetBlockMode(bool bBlock)
{
#ifdef FIONBIO   // 为了能够在SunOS下编译，SunOS没有定义FIONBIO
#ifdef WIN32
   unsigned long bmode;
...
int CTcpSocket::RecvPart(char *vptr, int nMaxSize, int timeout)
{
   int result;
   char szmsg[512];
   if(m_socket==INVALID_SOCKET) return -1;
   if(nMaxSize<=0) return -10;
   result = CheckDataArrived(timeout);
   if (result<=0)
      return(result); // -1:closed,-2:error & closed, 0:timeout, 1:have data received at the socket
   result = recv(m_socket,vptr,nMaxSize,0);
   if (result<0 && GetLastError(szmsg,sizeof(szmsg)-1)==EWOULDBLOCK)
      return(0);
   if (result<=0)
   {
      // ==0 : be gracefully closed by remote
      ...
      if (result==0)
         strcpy(szmsg,"0:be gracefully closed by remote!");
#ifdef WIN32
      _snprintf(vptr,nMaxSize,"%s",szmsg);
#else
      snprintf(vptr,nMaxSize,"%s",szmsg);
#endif
      Close();
      return(-3);
   }
...
int CTcpSocket::CheckDataArrivedUS(int ustimeout)
{
   int result=0;
   if (m_socket==INVALID_SOCKET) 
      return(-1);
...
int CTcpSocket::CheckSendAvailableUS(int ustimeout)
{
   int result=0;
   if (m_socket==INVALID_SOCKET) 
      return(-1);
...
int CTcpSocket::RecvPartUS(char *vptr, int nMaxSize, int ustimeout)
{
   int result;
   char szmsg[512];
   if(m_socket==INVALID_SOCKET) return -1;
   if(nMaxSize<=0) return -10;
   result = CheckDataArrivedUS(ustimeout);
   if (result<=0)
      return(result); // -1:closed,-2:error & closed, 0:timeout, 1:have data received at the socket
   result = recv(m_socket,vptr,nMaxSize,0);
   if (result<0 && GetLastError(szmsg,sizeof(szmsg)-1)==EWOULDBLOCK)
...
int CTcpSocket::CheckSRAvailableUS(int ustimeout)
{
   int result=0;
   if (m_socket==INVALID_SOCKET) 
      return(-1);
