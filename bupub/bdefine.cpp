...
#ifndef NOIPCS
#include "MsgQueue.h"
#define MAXBULINKS   150
CMQFactory g_MQFactory;
int BUSetMQName(const char *MQName)
{
   if (MQName[0]!='\0'&&g_MQFactory.New(MQName,MAXBULINKS))
      return(1);
   else
      return(0);
}
...
CSvrLink *BUPubInitialize(TBDefine *pBDefine,XCALLBDFUNC_M pCallFunc,XWRITEAPPFUNC pWriteApp,CLogFile *pLogFile,XPROCESS_M process)
{
   g_BDefines = pBDefine;
   g_pLogFile = pLogFile;
   g_SvrLink.Init(pCallFunc,pWriteApp,process);
   return(&g_SvrLink);
}
