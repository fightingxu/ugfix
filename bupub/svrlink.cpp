...
int CSvrLink::RecvMsgQ(int timeout)
{
   ...
   if (m_pMsgQueue==NULL || !(m_pMsgQueue->IsOpened()))
      return(-1);
   /* ****** Updated by CHENYH at 2010-2-4 16:09:47 ****** 
   return(m_pMsgQueue->Recv(buf,sizeof(buf),timeout,0));      
   *******************************************************/
   int rtn;
   MSGBUF *pMsgBuf=(MSGBUF *)buf;
   rtn = m_pMsgQueue->Recv(buf,sizeof(buf),timeout,0);
   if (rtn<(int)sizeof(MSGHEAD))
      return(rtn);
   if (pMsgBuf->head.msgid!=LMSGID || pMsgBuf->head.len>MAXMSG)
   {
      // 属于不正常的MSG，关闭端口，重新连接。。。
      // 或者数据流不同步，通过重新连接进行同步。。。
      DEBUG_RUNTIME_MSGOUT("属于不正常的MSG或者数据流不同步，关闭端口，重新连接\n");
   ...
   return(pMsgBuf->head.len+sizeof(MSGHEAD));
#endif   
}
...
int CSvrLink::RecvMsg(int timeout)
{
   ...
   else if (rtn>=(int)sizeof(MSGHEAD))
   {
      m_lastrcvtick = GetTickCount();
   }
   return(rtn);
...
GETNEXTMSG:
   ...
   if (rtn>=(int)(sizeof(MSGHEAD)))
   {
      ...
      case EXITMSGTYPE:
         ...
         break;
      case WRITEAPPINFO:
         if (pWriteAppInfo!=NULL)
            (*pWriteAppInfo)(bccBFuncNo,bccID);
         break;
      case MT_GETREQ:
...
int CSvrLink::PutRowData(ST_PACK *pdata)
{
   ...
   if (head.recCount<=0)
   {
	   CheckAnswerParmBits();  // Added at 2009-5-19 13:44:20
...
int CSvrLink::AnswerDataEx(TRUSERID *ruserid, int len, void *msgbuf)
{
   unsigned char sbuf[MAXMSG];
   RP_ANSDATAEXH *pad;
   unsigned char *buf;
   unsigned int buflen;
   if (len+sizeof(RP_ANSDATAEXH)>=MAXMSG)
      return(-101);
   pad = (RP_ANSDATAEXH *)sbuf;
   ...
   buflen = sizeof(RP_ANSDATAEXH)+len;
   memcpy(buf,msgbuf,len);
   return(SendMsg(MT_ANSDATAEX,buflen,sbuf)); 
}
...
int CSvrLink::ExtCall(int sourceno, int destno, int funcno, int batchno, int acktime, ST_CPACK *rpack, ST_CPACK *apack, ST_PACK *pArrays)
{
   ...
   while (rtn==0)
   {
      ...
      if (rtn>0)
      {
         ...
         {
            DEBUG_RUNTIME_MSGOUT("ExtCall中间收到了非MT_PUSHDATA应答包，属于程序错误!");
            // 2013/5/6 11:41:21 发现在特殊情况下出现这类问题，
            /*
            757460514 03:30:35 15112 Level666 -- MsgIn:411756-0|CYHUG1|ML
            757460732 03:30:35 15112 Level666 -- MsgIn:411756-0|CYHUG1|GL
            757461388 03:30:36 15112 Level1854 -- FIXSession(ML) do EOD
            757461388 03:30:36 15112 Level1856 -- FIXSession(ML) FE-EOD
            757461512 03:30:36 15112 Level1855 -- FIXSession(ML) EOD Done:1
            757461528 03:30:36 15112 Level1111 -- FILE:c:\dd\projects\fix_oms\01编码\ugate\bupub\svrlink.cpp LINE:1010 Msg:ExtCall中间收到了非MT_PUSHDATA应答包，属于程序错误!
            757461668 03:30:36 15112 Level1854 -- FIXSession(GL) do EOD
            757461668 03:30:36 15112 Level1856 -- FIXSession(GL) FE-EOD
            757461793 03:30:36 15112 Level1855 -- FIXSession(GL) EOD Done:1
            */
            // 为了跟踪特别加下面这个dump:  似乎总是在Session要求EOD的时候，会出现这个问题的:( 不过也无所谓的 
            //sprintf(szmsg,"XC-GPD%u.err",GetTickCount());
            //DataDumpFile(szmsg,(const char *)pmb,pmb->head.len+sizeof(MSGHEAD));  
            //////////////////////////////////////////////////////////////////////////
            rtn = 0; // 继续读取数据，以便接收到正确的MT_PUSHDATA应答包
         }
      }
...
int CSvrLink::BCCLogOut(char logtype, int logid, int textlen, char *text, int datalen, void *data)
{
   ...
   /* ****** Updated by CHENYH at 2009-2-26 11:19:33 ****** */
   // 为了安全特别增加如下判断:
   if (textlen<0||datalen<0||textlen+datalen>MAXMSG-(int)sizeof(RP_BCCLOG))
   {
      printf("Invalid BCCLogOut, textlen=%d,datalen=%d\n",textlen,datalen);
      return(0xFFFF);
   }
   /*********************************************************/
   ...
   pbl->logtext[textlen]='\0'; // added @ 2009-2-26 11:23:56
...
void CSvrLink::SetAnswerHead(TRUSERID *handle, ST_PACKHEAD *phead)
{
   memcpy(&head,phead,sizeof(ST_PACKHEAD));
   head.recCount = 0;
}

// 为了在PutRow等函数的EncodeRow中，不至于有意外发生，导致BU的Core Down
void CSvrLink::CheckAnswerParmBits()
{
	FilterParmBits(head.ParmBits);
}

int CSvrLink::GetSvrFuncRegister(int sourceno, int funcno)
{
   RP_GETSVRFUNCREG gsfr;
   int rtn;
   gsfr.SourceNo = htons(sourceno);
   gsfr.FuncNo = htons(funcno);
   rtn = SendMsg(MT_GETSVRFUNCREG,sizeof(gsfr),&gsfr);
   while (0==rtn)
   {
      rtn = RecvMsg(5000);
      if (rtn>0)
      {
         MSGBUF *pmb = GetRecvMsg();
         if (pmb->head.msgtype==MT_GETSVRFUNCREG)
         {
            memcpy(&rtn,pmb->data,sizeof(rtn));
            if (ntohl(1)!=1)
            {
               reverse(&rtn,sizeof(rtn));
            }
            return(rtn);
         }
         else
         {
            DEBUG_RUNTIME_MSGOUT("GetSvrFuncRegister中间收到了非MT_GETSVRFUNCREG的应答包，属于程序错误!");
            rtn = 0; // 继续取BCC的消息
         }
      }
      else if (rtn==0)
      {
         return(-1002);  // 等待BCC返回失败
      }
   }
   return(-1001);  // 属于BU/BCC之间数据交换的异常
}

int CSvrLink::GetReqUserID(TRUSERID *handle, char *sRUserID)
{
   if (handle==NULL)
   {
      handle = &ruserid;
   ...
   unsigned int i,off;
   unsigned char *pusrid = (unsigned char *)(handle->syshead.buf);
   sprintf(sRUserID,"%04X",handle->SourceNo);  // SourceNo
   off = 4;
   for (i=0;i<sizeof(SYSHEAD);++i)
   {
      sprintf(sRUserID+off,"%02X",pusrid[i]);
      off += 2;
   }
   return(off);
}
...
int CSvrLink::PostBookDataX(unsigned int postdno, const TRUSERID *hTRUserID, void *pdata, int pdlen, char pushmode, int acktime)
{
   unsigned char sbuf[MAXMSG];
   RP_POSTBDATA *pbd=(RP_POSTBDATA *)sbuf;
   unsigned short len;

   pbd->pushmode = pushmode;
   pbd->acktime = htonl(acktime);
   pbd->PostDestID = htonl(postdno);
   if (hTRUserID==NULL)
   {
      pbd->SourceNo = htons(0xFFFF);
   }
   else
   {
      pbd->SourceNo = htons(hTRUserID->SourceNo);
      memcpy(&(pbd->syshead),&(hTRUserID->syshead),sizeof(pbd->syshead));
   }
   len = sizeof(RP_POSTBDATA);
   if (pdlen+len>MAXMSG||pdlen<0)
      return(-1000);
   if (pdlen>0 && pdata!=NULL)
   {
      memcpy(sbuf+len,pdata,pdlen);
      len += pdlen;
   }
   return(SendMsg(MT_POSTBDATA,len,sbuf));
}

unsigned char HexChar2UC(char cHex)
{
   if (cHex>='0' && cHex<='9')
      return(cHex-'0');
   if (cHex>='a' && cHex<='f')
...
int CSvrLink::PostBookData(unsigned int postdno, const char *sRUserID, void *pdata, int pdlen, char pushmode, int acktime)
{
   TRUSERID userid;
   if (sRUserID==NULL)
      return(PostBookDataX(postdno,NULL,pdata,pdlen,pushmode,acktime));
   unsigned char *pusrid = (unsigned char * )(userid.syshead.buf);
   unsigned int i,off;
   unsigned char uc;
   userid.SourceNo = 0;
   for (off=0;off<4;++off)
   {
      uc = HexChar2UC(sRUserID[off]);
      if (uc>0x0F)
         return(-3);
      userid.SourceNo = (userid.SourceNo<<4)|uc;
   }
   for (i=0;i<sizeof(SYSHEAD);++i)
   {
      pusrid[i] = HexChar2UC(sRUserID[off++]);
      if (pusrid[i]>0x0F)
         return(-3);
      uc = HexChar2UC(sRUserID[off++]);
      pusrid[i] = (pusrid[i]<<4)|uc;
   }
   return(PostBookDataX(postdno,(const TRUSERID *)pusrid,pdata,pdlen,pushmode,acktime));
}


// 函数名: CSvrLink::CheckPushPack
// 编程  : 陈永华 2010-12-2 12:20:55
// 描述  : 检查请求数据是否是属于推送过来的请求包
// 返回  : int : 1 - PM1, 1类推送数据包；2 - 请求功能的推送消息；3 - 应答功能的推送消息；0 - 其他类型
// 参数  : char *oQueueID16 [OUT]: 用于获取推送请求的QueueID，这个用于匹配原始请求和应答的ID
int CSvrLink::CheckPushPack(char *oQueueID16)
{
   // 通过检测 reqhead中的hook.queuename来判断:
   strcpy(oQueueID16,reqhead.hook.queuename);
   if (memcmp(reqhead.hook.hostname+PMMARK_HOSTNAME,"PM0",3)==0)
   {
      if (memcmp(reqhead.hook.queuename+P_QN_MARK,BUASW_MARK,L_QN_MARK)==0)
         return(3);  // 应答功能的推送消息
      if (memcmp(reqhead.hook.queuename+P_QN_MARK,BUREQ_MARK,L_QN_MARK)==0)
         return(2);  // 请求功能的推送消息
   }
   else if(memcmp(reqhead.hook.hostname+PMMARK_HOSTNAME,"PM1",3)==0)
   {
      return(1);  // PM1, 1类推送数据包
   }
   return(0); // 其他不符合推送功能数据包类型的，就归结于此
}

void CSvrLink::Init(XCALLBDFUNC_M pCallFunc, XWRITEAPPFUNC pWriteApp, XPROCESS_M process)
{
   pProcess=process;   
